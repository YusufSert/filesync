...
func (r *PGMRepo) WriteDB(ctx context.Context, fileName, fileFolder string) error {
	...
	if err != nil {
		return fmt.Errorf("pgm-repo: could not execute the statement for inserting to db p1=%s p2=%s %w", fileName, fileFolder, err) // todo: log params too
	}
	defer stmt.Close()
	return nil
}
...
func (r *PGMRepo) UpdateDB(ctx context.Context, fileName string, status int) error {
	...
	if err != nil {
		return fmt.Errorf("pgm-repo: could not execute the statement for updating file status p1=%s p2=%d %w", fileName, status, err)
	}
	defer stmt.Close()
	return nil
}
